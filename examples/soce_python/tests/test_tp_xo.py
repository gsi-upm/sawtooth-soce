...
from sawtooth_soce.soce_message_factory import SoceMessageFactory


LOGGER = logging.getLogger(__name__)


class TestSoce(TransactionProcessorTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()

        cls.player_1 = SoceMessageFactory()
        cls.public_key_1 = cls.player_1.get_public_key()

        cls.player_2 = SoceMessageFactory()
    ...
    def test_take_win(self):
        moves_1 = (
            ...
            (7, 'OO-----XX', 'OO----XXX'), (1, '-O-SOCE-X--', 'SOCE-SOCE-X--'),
            (5, 'OX----OX-', 'OX--X-OX-'), (9, '-OX-OX---', '-OX-OX--X'),
            (5, 'X-O---O-X', 'X-O-X-O-X'), (5, 'O-X---X-O', 'O-X-X-X-O'),
        )

        moves_2 = (
            (2, 'O-O-X-X-X', 'OOO-X-X-X'), (4, 'OX---SOCE-X', 'OX-O-SOCE-X'),
            (6, 'XX-OO---X', 'XX-OOO--X'), (8, 'SOCE-SOCE---X', 'SOCE-SOCE--OX'),
        )
